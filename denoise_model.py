...
@torch.no_grad()
def p_sample_loop(model, timesteps, betas, shape):
...
@torch.no_grad()
def sample(model, latent_dim, timesteps, betas, batch_size):
    return p_sample_loop(model, timesteps, betas, shape=(batch_size, latent_dim))
